...
let rec interpret () = print_string "> ";
  match try_read_line () with
    |Some(line) -> begin match String.sub line 0 (String.length line) with
      |"#exit" -> ()
      |s -> 
        try
          print_endline (interpretStr s); interpret () 
        with Failure(es) -> print_endline ("[Error] " ^ es); interpret () end
    |None -> print_newline ();;

print_newline ();;
print_string "Mini-Ocaml Interpreter v0.1\n";;
print_string "by Johannes Schöneberger\n";;
print_newline ();;
